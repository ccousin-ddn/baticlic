...
**/
	require_once (dirname(__FILE__)."/../models/m_mem_memo.php");
	
	class mem_memo_view extends mem_memo_model {
		...
		public function __construct()
		{
			...
			$this->columns = array(
				array("field"=>"job_name", "alter"=>"", "title"=>html("Chantier"), "sortable"=>true, "searchable"=>true, "class"=>"text-nowrap", "halign"=>"left", "align"=>"left"),
				array("field"=>"mem_date", "alter"=>"date-be", "title"=>html("Date"), "sortable"=>true, "searchable"=>true, "class"=>"text-nowrap", "halign"=>"left", "align"=>"left", "sorter"=>"sortDate"),
				array("field"=>"usr_firstname", "alter"=>"", "title"=>html("Utilisateur"), "sortable"=>true, "searchable"=>true, "class"=>"text-nowrap", "halign"=>"left", "align"=>"left"),
				array("field"=>"mem_text", "alter"=>"", "title"=>html("Description"), "sortable"=>false, "searchable"=>true, "class"=>"text-break", "halign"=>"left", "align"=>"left"),
			);
		...
		public function v_createCard($doc_document="")
		{
			...
			//$inputs = $input->create("text", "Code", "typ_code", false, $this->dataSent['newValue']??"", "col-md-3");
			$inputs = $input->create("select", "Chantier", "idjob", true, "", "col-md-8", "", array("idlist"=>"6","table"=>"job_job","search"=>"true","filterChild"=>null,"filtering"=>true));
			$inputs .= $input->create("date", "Date", "mem_date", false, "", "col-md-2");
			$inputs .= $input->create("select", "Utilisateur", "iduser", false, "", "col-md-2", "", array("idlist"=>"1","table"=>"usr_user","search"=>"true","filterChild"=>null,"filtering"=>true));
			$inputs .= $input->create("textArea", "Mémo", "mem_text", false, "", "col-md-12", "", array("row"=>5));
			...
			$btn_delete = $button->create("deleteAllLevel", "btn-left");
			$btn_pdf	= "";
			...
			// *** Title variable ***
			$title = $this->values->typ_name ?? gettext("Nouveau ".$this->label);
			// *** Page object ***
			// info : new page(type, id, label, pageClass, body, buttons)
			$page = new page("modal", $this->table."_modal", $this->picto." ".$title, "modal-lg no-scroll", $body, $btn_save.$btn_close.$btn_delete);
