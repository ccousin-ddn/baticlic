...
**/
	require_once (dirname(__FILE__)."/../models/m_inv_tracking.php");
	
	class inv_tracking_view extends inv_tracking_model {
		...
		public function __construct()
		{
			...
			$this->columns = array(
				array("field"=>"tra_date", "alter"=>"date-be", "title"=>html("Date"), "sortable"=>true, "searchable"=>true, "class"=>"text-nowrap", "halign"=>"left", "align"=>"left", "width"=>"150", "sorter"=>"sortDate", "card-visible"=>"true"),
				array("field"=>"tra_action", "alter"=>"", "title"=>html("Action"), "sortable"=>true, "searchable"=>true, "class"=>"text-break", "halign"=>"left", "align"=>"left", "sorter"=>"", "card-visible"=>"true"),
				array("field"=>"usr_firstname", "alter"=>"readonly", "title"=>html("Editeur"), "sortable"=>true, "searchable"=>true, "class"=>"text-break", "halign"=>"left", "align"=>"left", "sorter"=>"", "card-visible"=>"true"),
				
			);
			$this->columns_edit = array(
				array("field"=>"tra_date", "alter"=>"", "title"=>html("Date"), "sortable"=>true, "searchable"=>true, "class"=>"text-nowrap", "halign"=>"left", "align"=>"left", "sorter"=>"sortDate", "card-visible"=>"true"),
				array("field"=>"tra_action", "alter"=>"", "title"=>html("Action"), "sortable"=>true, "searchable"=>true, "class"=>"text-nowrap", "halign"=>"left", "align"=>"left", "sorter"=>"", "card-visible"=>"true"),
				array("field"=>"usr_firstname", "alter"=>"", "title"=>"", "sortable"=>true, "searchable"=>true, "class"=>"text-nowrap", "halign"=>"left", "align"=>"left", "sorter"=>"", "card-visible"=>"true"),
		...
		public function v_editLine()
		{
			...
			$this->values->tra_date = $input->create("date", "", "tra_date", false, "", "", "form-normal", array("horizontal"=>"left","vertical"=>"bottom"));
			$this->values->tra_action = $input->create("textarea", "", "tra_action", false, "", "", "form-normal", array("row"=>1));
			$this->values->usr_firstname = $input->create("readonly", "", "usr_firstname", false, $_SESSION['usr_firstname']??"", "", "form-normal");
		...
		public function v_createCard($doc_document="")
		{
			// *** Input object ***
			$input = new input($this->values, $this->cryptfields);
			// info : input->create(type, label, name, mandatory(true/false)[, defaultValue[, divSize[, inputClass[, info(array)]]]]) ,"none-result-text"=>gettext("Créer le tra_name")
			...
			$inputs .= $input->create("textarea", gettext("Action"), "tra_action", false, "", "col-md-12", "", array("row"=>2));
			$inputs .= $input->create("date", gettext("Date"), "tra_date", false, "", "col-md-3", "", array("horizontal"=>"left","vertical"=>"bottom"));
			...
			// $tabs = $tab->create("tracking", "underline ml-2", $forms);
		...
		{
			//$type="view";
			$pdf_name = gettext("")." ".$this->values->tra_name;
