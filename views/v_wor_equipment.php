...
**/
	require_once (dirname(__FILE__)."/../models/m_wor_equipment.php");
	
	class wor_equipment_view extends wor_equipment_model {
		...
		public function __construct()
		{
			...
			$this->columns = array(
				array("field"=>"equ_date_in", "alter"=>"date-be", "title"=>html("Date"), "sortable"=>true, "searchable"=>true, "class"=>"text-nowrap", "halign"=>"left", "align"=>"left", "sorter"=>"sortDate", "card-visible"=>"true"),
				array("field"=>"equ_price", "alter"=>"euro", "title"=>html("Prix achat"), "sortable"=>true, "searchable"=>true, "class"=>"text-nowrap", "halign"=>"left", "align"=>"left", "sorter"=>"sortEuro", "card-visible"=>"true"),
				array("field"=>"equ_qty", "alter"=>"", "title"=>html("Quantité"), "sortable"=>true, "searchable"=>true, "class"=>"text-nowrap", "halign"=>"left", "align"=>"left", "sorter"=>"", "card-visible"=>"true"),
				array("field"=>"idarticle", "alter"=>"", "title"=>html("Article"), "sortable"=>true, "searchable"=>true, "class"=>"text-nowrap", "halign"=>"left", "align"=>"left", "sorter"=>"", "card-visible"=>"true"),
			...
			$this->columns_small = array(
				...
				array("field"=>"equ_qty", "alter"=>"", "title"=>html("Quantité"), "sortable"=>true, "searchable"=>false, "class"=>"text-nowrap font-weight-bold", "halign"=>"left", "align"=>"right"),
				array("field"=>"equ_price", "alter"=>"euro", "title"=>html("Prix achat"), "sortable"=>true, "searchable"=>false, "class"=>"text-nowrap", "halign"=>"left", "align"=>"right"),
				array("field"=>"equ_date_in", "alter"=>"date-be", "title"=>html("Date"), "sortable"=>true, "searchable"=>false, "class"=>"text-nowrap", "halign"=>"left", "align"=>"right", "sorter"=>"sortDate"),
			);
		...
		public function v_createLineTable($idparent)
		{
			...
			$table = new table($this->table, $this->level, false, array("noaction"=>false,"read"=>false,"edit"=>true,"delete"=>true));
			// *** create columns title ***
			...
			// *** create table ***
			$param = array("height"=>"auto", "show-columns"=>"false", "pagination"=>"false", "search"=>"true", "new-recordtable"=>"false", "id-parent"=>"$idparent", "classes"=>"table table-sm table-card", "striped"=>"false", "export-table"=>"false", "filter-table"=>"");
		...
		public function v_createCard($doc_document="")
		{
			// *** Input object ***
			$input = new input($this->values, $this->cryptfields);
			// info : input->create(type, label, name, mandatory(true/false)[, defaultValue[, divSize[, inputClass[, info(array)]]]]) ,"none-result-text"=>gettext("Créer le equ_name")
			...
			$inputs .= $input->create("date", gettext("Date"), "equ_date_in", false, "", "col-md-3", "", array("horizontal"=>"right","vertical"=>"bottom"));
			$inputs .= $input->create("text", gettext("Prix achat"), "equ_price", false, "", "col-md-6");
			$inputs .= $input->create("select", gettext("Quantité"), "equ_qty", false, "", "col-md-6", "", array("idlist"=>"1","table"=>"tableselect","search"=>"true","filterChild"=>null,"filtering"=>false));
			$inputs .= $input->create("select", gettext("Article"), "idarticle", false, "", "col-md-6", "", array("idlist"=>"1","table"=>"tableselect","search"=>"true","filterChild"=>null,"filtering"=>false));
			...
			// $tabs = $tab->create("Equipement", "underline ml-2", $forms);
		...
		{
			//$type="view";
			$pdf_name = gettext("")." ".$this->values->equ_name;
