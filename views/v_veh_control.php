...
**/
	require_once (dirname(__FILE__)."/../models/m_veh_control.php");
	
	class veh_control_view extends veh_control_model {
		...
		public function __construct()
		{
			...
			$this->columns = array(
				...
				array("field"=>"con_type", "alter"=>"array", "title"=>html("Type"), "sortable"=>true, "searchable"=>true, "class"=>"text-nowrap", "halign"=>"left", "align"=>"left", "sorter"=>"", "card-visible"=>"true"),
				...
				array("field"=>"con_date_done", "alter"=>"date-be", "title"=>html("Fait le"), "sortable"=>true, "searchable"=>true, "class"=>"text-nowrap", "halign"=>"left", "align"=>"left", "sorter"=>"sortDate", "card-visible"=>"true"),
				array("field"=>"con_mileage", "alter"=>"", "title"=>html("Kilométrage"), "sortable"=>true, "searchable"=>true, "class"=>"text-nowrap", "halign"=>"left", "align"=>"left", "sorter"=>"", "card-visible"=>"true"),
			...
			$this->color_conds = array(
				array("column"=>"con_done", "query"=>" == 0", "class"=>"td-danger"),
				array("column"=>"con_done", "query"=>" == 1", "class"=>"td-success")
			);
		}
		...
		public function v_createFullTable()
		{
			...
			// *** create table ***
			$param = array("height"=>"auto", "show-columns"=>"false", "pagination"=>"true", "search"=>true, "classes"=>"table table-no-bordered table-hover table-light", "striped"=>"false", "new-record"=>"true", "export-table"=>"false", "filter-table"=>"con_type,con_done");
		...
		public function v_createCard($doc_document)
		{
			...
			$inputs .= $input->create("select", gettext("Type"), "con_type", true, "", "col-md-6", "", array("contents"=>$this->con_type));
			$inputs .= $input->create("date", gettext("Date"), "con_date", true, "", "col-md-3", "", array("horizontal"=>"right","vertical"=>"bottom"));
			$inputs .= $input->create("radio", gettext("Fait"), "con_done", false, "", "col-md-3", "", array("contents"=>$this->yesno));
			$inputs .= $input->create("date", gettext("Fait le"), "con_date_done", false, "", "col-md-3", "", array("horizontal"=>"right","vertical"=>"bottom"));
			$inputs .= $input->create("number", gettext("Kilométrage"), "con_mileage", false, "", "col-md-3", "", array("pattern"=>"[0-9]{6}", "max"=>999999));
			$inputs .= $input->create("textarea", gettext("Commentaire"), "con_remark", false, "", "col-md-6 offset-md-6", "", array("row"=>2));
			...
			$btn_createNext = '
					<button type="button" id="btn_veh_control" class="btn btn-outline-info" onclick="iud(\''.$this->table.'\',\''.$this->idrecord.'\',\'update\',\'none\', function(){cardAction(\''.$this->table.'\',\''.$this->idrecord.'\',\'createNext\',\'veh_control\')})" autocomplete="off" data-loading-text="<i class=\'fas fa-circle-notch fa-spin mr-2\'></i>'.gettext("En cours...").'">
						<i class="fal fa-calendar-day mr-2"></i>
						'.gettext("Créer le prochain contrôle").'
					</button>
			';
			...
			// $tabs = $tab->create("Contrôle", "underline ml-2", $forms);
		...
		public function v_createToast()
		{
			...
			foreach($this->values as $control){
				$toast->fa = "fa-ballot-check";
				$toast->title = $control['veh_name'];
				$toast->time = alterData("date-be",$control['con_date']);
				$toast->body = $this->con_type[$control['con_type']];
