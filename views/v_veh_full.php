...
**/
	require_once (dirname(__FILE__)."/../models/m_veh_full.php");
	
	class veh_full_view extends veh_full_model {
		...
		public function __construct()
		{
			...
			$this->columns = array(
				...
				array("field"=>"usr_name", "alter"=>"", "title"=>html("Utilisateur"), "sortable"=>true, "searchable"=>true, "class"=>"text-nowrap", "halign"=>"left", "align"=>"left", "sorter"=>"", "card-visible"=>"true"),
				array("field"=>"ful_date", "alter"=>"date-be", "title"=>html("Date"), "sortable"=>true, "searchable"=>true, "class"=>"text-nowrap", "halign"=>"left", "align"=>"left", "sorter"=>"sortDate", "card-visible"=>"true"),
				array("field"=>"ful_mileage", "alter"=>"", "title"=>html("Kilométrage"), "sortable"=>true, "searchable"=>true, "class"=>"text-nowrap", "halign"=>"left", "align"=>"left", "sorter"=>"", "card-visible"=>"true"),
				array("field"=>"ful_liter", "alter"=>"", "title"=>html("Litres"), "sortable"=>true, "searchable"=>true, "class"=>"text-nowrap", "halign"=>"left", "align"=>"left", "sorter"=>"", "card-visible"=>"true"),
				array("field"=>"ful_amount", "alter"=>"", "title"=>html("Montant"), "sortable"=>true, "searchable"=>true, "class"=>"text-nowrap", "halign"=>"left", "align"=>"left", "sorter"=>"", "card-visible"=>"true"),
				array("field"=>"ful_consum", "alter"=>"", "title"=>html("L/100 Km"), "sortable"=>true, "searchable"=>true, "class"=>"text-nowrap", "halign"=>"left", "align"=>"left", "sorter"=>"", "card-visible"=>"true"),
		...
		public function v_createCard($doc_document)
		{
			...
			// info : input->create(type, label, name, mandatory(true/false)[, defaultValue[, divSize[, inputClass[, info(array)]]]]) ,"none-result-text"=>gettext("Créer le ful_name")
			...
			$inputs .= $input->create("select", gettext("Utilisateur"), "iduser", false, "", "col-md-6", "", array("idlist"=>"2","table"=>"usr_user","search"=>"true","filterChild"=>null,"filtering"=>false));
			$inputs .= $input->create("date", gettext("Date"), "ful_date", true, "", "col-md-3", "", array("horizontal"=>"right","vertical"=>"bottom"));
			$inputs .= $input->create("number", gettext("Kilométrage"), "ful_mileage", true, "", "col-md-3", "", array("pattern"=>"[0-9]{6}", "max"=>999999));
			$inputs .= $input->create("number", gettext("Litres"), "ful_liter", true, "", "col-md-3", "", array("pattern"=>"[0-9]{3}", "max"=>999));
			$inputs .= $input->create("text", gettext("Montant"), "ful_amount", false, "", "col-md-3");
			$inputs .= $input->create("readonly", gettext("Ancien Kilométrage"), "ful_old_mileage", true, "", "col-md-3 offset-md-3", "", array("pattern"=>"[0-9]{6}", "max"=>999999));
			$inputs .= $input->create("readonly", gettext("L/100 Km"), "ful_consum", true, "", "col-md-3", "", array("pattern"=>"[0-9]{6}", "max"=>999999));
			...
			// $tabs = $tab->create("Carburant", "underline ml-2", $forms);
		...
		public function v_createPdf($type)
		{
			...
			$pdf_name = gettext("")." ".$this->values->ful_name.'.pdf';
			
			$this->json['html'] = $pdfDoc->createPdf($this->values,$pdf_name,$type);
			$this->json['subject'] = gettext("")." ".$this->values->ful_name;
		}
