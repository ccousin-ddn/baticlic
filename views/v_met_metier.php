<?php
/**
*** 05-2023@SOLUfile SRL 
**/
	require_once (dirname(__FILE__)."/../models/m_met_metier.php");
	
	class met_metier_view extends met_metier_model {
		...
		public function __construct()
		{
			...
			$this->columns = array(
				array("field"=>"idmetier", "alter"=>"", "title"=>html("ID"), "sortable"=>true, "searchable"=>true, "class"=>"text-nowrap", "halign"=>"left", "align"=>"left", "sorter"=>"", "card-visible"=>"true"),
				array("field"=>"met_name", "alter"=>"", "title"=>html("Dénomination"), "sortable"=>true, "searchable"=>true, "class"=>"text-nowrap", "halign"=>"left", "align"=>"left", "sorter"=>"", "card-visible"=>"true"),
				array("field"=>"met_rate", "alter"=>"euro", "title"=>html("Taux horaire"), "sortable"=>true, "searchable"=>false, "class"=>"text-nowrap", "halign"=>"left", "align"=>"left", "sorter"=>"", "card-visible"=>"true"),
				array("field"=>"met_rate_brut", "alter"=>"euro", "title"=>html("Taux horaire brut"), "sortable"=>true, "searchable"=>false, "class"=>"text-nowrap", "halign"=>"left", "align"=>"left", "sorter"=>"", "card-visible"=>"true"),
		...
		public function v_createCard($doc_document="")
		{
			// *** Input object ***
			$input = new input($this->values, $this->cryptfields);
			// info : input->create(type, label, name, mandatory(true/false)[, defaultValue[, divSize[, inputClass[, info(array)]]]]) ,"none-result-text"=>gettext("Créer le met_name")
			...
			$inputs .= $input->create("text", gettext("Dénomination"), "met_name", false, "", "col-md-6");
			$inputs .= $input->create("text", gettext("Taux horaire"), "met_rate", false, "", "col-md-3");
			$inputs .= $input->create("text", gettext("Taux horaire brut"), "met_rate_brut", false, "", "col-md-3");
			...
			// $tabs = $tab->create("Métier", "underline ml-2", $forms);
		...
		public function v_createPdf($type)
		{
			...
			$pdf_name = gettext("")." ".$this->values->met_name.'.pdf';
			
			$this->json['html'] = $pdfDoc->createPdf($this->values,$pdf_name,$type);
			$this->json['subject'] = gettext("")." ".$this->values->met_name;
		}
