...
**/
	...
	class sto_stock_model extends crud {
		
		public function __construct()
		{
			parent::__construct();
			
			$this->table 		= "sto_stock";
			$this->key			= "idstock";
			$this->duplicateKey = false;
			
			$this->fields 		= "sto_stock.*, art_code, art_description, art_price, art_unit, (art_price*sto_quantity) AS total";
			...
			$this->conds 		= array();
			$this->groups 		= array();
			$this->orders 		= array("art_description");
			$this->fieldName	= "sto_name";
		...
		{
			$this->conds = array("idstock = ".$idrecord);
			//$this->orders = array();
			//$this->debugging = true;
		...
		public function m_getList($id, $conds=array())
		{
			$this->conds = $conds;
			switch($id){
				case 1 : $this->fields = "idstock AS id, sto_name AS val, '' AS tokens"; $this->select(); break;
				case "mvt" : $this->fields = "idarticle AS id, art_code AS val, art_description AS tokens"; $this->orders = array("art_code");$this->select(); break;
			}
			return true;
		}
		
		public function m_updateQty($idshop, $idarticle, $qty)
		{
			$query = "
			UPDATE sto_stock
			SET sto_quantity = sto_quantity-$qty
			WHERE idshop = $idshop AND idarticle = $idarticle
			";
			//$this->debugging = true;
