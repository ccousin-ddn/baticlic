...
**/
	require_once (dirname(__FILE__)."/../views/v_abs_type.php");
	
	class abs_type extends abs_type_view {
		
		public function __construct()
		{
			parent::__construct();
			
			$this->label 		= gettext("Type d'absence");
			$this->labels 		= gettext("Types d'absence");
			$this->newtext		= gettext("Nouveau type");
			$this->picto 		= '<i class="fal fa-clipboard-check"></i>';
			$this->level		= 1;
		...
		public function c_newLine(){
			if($this->checkUserRight()){
				...
				if($this->m_insert(http_build_query($this->values))){
					...
					$this->json['info'] = getText("Type d'absence ajouté");
					// *** view ***
					$this->values = array($this->values);
					$this->v_createTr();
					return $this->json;
			...
			if($this->checkUserRight()){
				...
				if($this->m_update(decrypt($this->idrecord), $this->dataSent)){
					...
					if($this->m_getById(decrypt($this->idrecord))){
						...
						$this->json['info'] = getText("Type d'absence mis à jour");
						return $this->json;
					}else{
		...
		public function c_delete(){
			if($this->checkUserRight()){
				if($this->m_delete(decrypt($this->idrecord))){
					...
					$this->json['info'] = getText("Type d'absence supprimé");
					return $this->json;
				}else{
