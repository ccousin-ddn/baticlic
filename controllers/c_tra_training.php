...
**/
	require_once (dirname(__FILE__)."/../views/v_tra_training.php");
	
	class tra_training extends tra_training_view {
		
		public function __construct()
		{
			parent::__construct();
			
			$this->label 		= gettext("Formation");
			$this->labels 		= gettext("Formations");
			$this->newtext		= gettext("Nouvelle Formation");
			$this->picto 		= '<i class="fal fa-users-class fa-fw"></i>';
			$this->level		= 1;
		...
		public function c_newLine(){
			if($this->checkUserRight()){
				...
				if($this->m_insert($this->values)){
					...
					$this->json['info'] = getText("Formation ajouté");
					// *** view ***
			...
			if($this->checkUserRight()){
				...
				if($this->m_update(decrypt($this->idrecord), $data)){
					...
					if($this->m_getById(decrypt($this->idrecord))){
						...
						$this->json['info'] = getText("Formation mis à jour");
						return $this->json;
					}else{
		...
		public function c_delete(){
			if($this->checkUserRight()){
				if($this->m_delete(decrypt($this->idrecord))){
					...
					$this->json['info'] = getText("Formation supprimé");
					return $this->json;
				}else{
