...
**/
	require_once (dirname(__FILE__)."/../views/v_wor_article.php");
	
	class wor_article extends wor_article_view {
		...
		public function __construct()
		{
			...
			$this->newtext		= gettext("Nouveau Article");
		...
		public function c_tableByParent(){
			if($this->checkUserRight()){
				// *** model ***
				$this->conds[] = "idwork = ".decrypt($this->idrecord);
				if($this->m_getAll()){
					// *** view ***
		...
		public function c_newLine(){
			if($this->checkUserRight()){
				// *** model ***
				$this->m_newRecord();
				$this->values->idwork = decrypt($this->idrecord);
				...
				if($this->m_insert($this->values)){
					...
					// *** view ***
					$this->values['idworarticle'] = $this->result['idparent'];
		...
		public function c_search(){
			if($this->checkUserRight()){
				...
				$this->conds = array("LOWER(art_name) LIKE '$search'");
				$this->dataSent = array();
		...
		public function c_selectSearch(){
			if($this->checkUserRight()){
				...
				$this->fields = $this->key." AS id, art_name AS val, '' AS tokens";
				$this->conds = array("LOWER(art_name) LIKE '$search'");
				$this->dataSent = array();
				if($this->m_getAll(false)){
					// *** view ***
