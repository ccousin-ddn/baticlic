...
**/
	require_once (dirname(__FILE__)."/../views/v_inv_line_previous.php");
	
	class inv_line_previous extends inv_line_previous_view {
		
		public function __construct()
		{
			parent::__construct();
			
			$this->label 		= gettext("Ligne facture déduction situation précédente");
			$this->labels 		= gettext("Ligne facture déduction situation précédentes");
			$this->newtext		= gettext("Nouveau Ligne facture déduction situation précédente");
		...
		public function c_tableByParent(){
			if($this->checkUserRight()){
				// *** model ***
				$this->conds[] = "idprevinvoice = ".decrypt($this->idrecord);
				//$this->debugging = true;
				if($this->m_getAll()){
					// *** view ***
		...
		public function c_newLine(){
			if($this->checkUserRight()){
				...
				if($this->m_insert($this->values)){
					...
					$this->json['info'] = getText("Ligne situation précédente ajouté");
					// *** view ***
		...
		public function c_insert(){
			if($this->checkUserRight()){
				...
				if($this->m_insert($data)){
					...
					$this->json['idparent'] = encrypt($this->result['idparent']);
					$this->json['info'] = getText("Ligne situation précédente ajoutée");
					return $this->json;
				}else{
			...
			if($this->checkUserRight()){
				...
				if($this->m_update(decrypt($this->idrecord), $data)){
					...
					if($this->m_getById(decrypt($this->idrecord))){
						...
						//$this->json['code'] = "lineUpdated";
						$this->json['info'] = getText("Ligne situation précédente mise à jour");
						return $this->json;
					}else{
		...
		public function c_delete(){
			if($this->checkUserRight()){
				if($this->m_delete(decrypt($this->idrecord))){
					...
					$this->json['info'] = getText("Ligne facture déduction situation précédente supprimé");
					return $this->json;
				}else{
