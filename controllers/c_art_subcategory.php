...
**/
	require_once (dirname(__FILE__)."/../views/v_art_subcategory.php");
	
	class art_subcategory extends art_subcategory_view {
		
		public function __construct()
		{
			parent::__construct();
			
			$this->label 		= gettext("Sous-catégorie");
			$this->labels 		= gettext("Sous-catégories");
			$this->newtext		= gettext("Nouvelle sous-catégorie");
			$this->picto 		= '<i class="fal fa-list-ul"></i>';
			$this->level		= 1;
		...
		public function c_tableByParent(){
			if($this->checkUserRight()){
				// *** model ***
				$this->conds = array("idcategory = ".decrypt($this->idrecord));
				if($this->m_getAll()){
					// *** view ***
		...
		public function c_newLine(){
			if($this->checkUserRight()){
				...
				if($this->m_insert(http_build_query($this->values))){
					...
					$this->json['info'] = getText("Sous-catégorie ajouté");
					// *** view ***
					$this->values = array($this->values);
					$this->v_createTr();
					return $this->json;
		...
		public function c_search(){
			if($this->checkUserRight()){
				// *** model ***
				$this->conds = array("sub_name LIKE '%".$this->dataSent['value']."%'");
				$this->dataSent = array();
			...
			if($this->checkUserRight()){
				...
				if($this->m_update(decrypt($this->idrecord), $this->dataSent)){
					...
					if($this->m_getById(decrypt($this->idrecord))){
						...
						$this->json['info'] = getText("Sous-catégorie mis à jour");
						return $this->json;
					}else{
		...
		public function c_delete(){
			if($this->checkUserRight()){
				if($this->m_delete(decrypt($this->idrecord))){
					...
					$this->json['info'] = getText("Sous-catégorie supprimé");
					return $this->json;
				}else{
