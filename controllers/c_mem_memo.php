...
**/
	require_once (dirname(__FILE__)."/../views/v_mem_memo.php");
	
	class mem_memo extends mem_memo_view {
		
		public function __construct()
		{
			parent::__construct();
			
			$this->label 		= gettext("Mémo");
			$this->labels 		= gettext("Mémos");
			$this->newtext		= gettext("Nouveau mémo");
			$this->picto 		= '<i class="fal fa-comment-alt-edit"></i>';
			$this->level		= 1;
		...
		public function c_newLine(){
			if($this->checkUserRight()){
				...
				if($this->m_insert(http_build_query($this->values))){
					...
					$this->json['info'] = getText("Mémo ajouté");
					// *** view ***
					$this->values = array($this->values);
					$this->v_createTr();
					return $this->json;
		...
		public function c_search(){
			if($this->checkUserRight()){
				// *** model ***
				$this->conds = array("mem_text LIKE '%".$this->dataSent['value']."%'");
				$this->dataSent = array();
			...
			if($this->checkUserRight()){
				...
				if($this->m_update(decrypt($this->idrecord), $this->dataSent)){
					...
					if($this->m_getById(decrypt($this->idrecord))){
						...
						$this->json['info'] = getText("Mémo mis à jour");
						return $this->json;
					}else{
		...
		public function c_delete(){
			if($this->checkUserRight()){
				if($this->m_delete(decrypt($this->idrecord))){
					...
					$this->json['info'] = getText("Mémo supprimé");
					return $this->json;
				}else{
