...
**/
	require_once (dirname(__FILE__)."/../views/v_wor_training.php");
	
	class wor_training extends wor_training_view {
		
		public function __construct()
		{
			parent::__construct();
			
			$this->label 		= gettext("Parcours formation");
			$this->labels 		= gettext("Parcours formations");
			$this->newtext		= gettext("Nouveau Parcours formation");
			$this->picto 		= '<i class="fal fa-user-graduate fa-fw"></i>';
			$this->level		= 1;
		...
		public function c_newLine(){
			if($this->checkUserRight()){
				...
				if($this->m_insert($this->values)){
					...
					$this->json['info'] = getText("Parcours formation ajouté");
					// *** view ***
			...
			if($this->checkUserRight()){
				...
				if($this->m_update(decrypt($this->idrecord), $data)){
					...
					if($this->m_getById(decrypt($this->idrecord))){
						...
						$this->json['info'] = getText("Parcours formation mis à jour");
						return $this->json;
					}else{
		...
		public function c_delete(){
			if($this->checkUserRight()){
				if($this->m_delete(decrypt($this->idrecord))){
					...
					$this->json['info'] = getText("Parcours formation supprimé");
					return $this->json;
				}else{
