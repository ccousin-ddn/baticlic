...
**/
	require_once (dirname(__FILE__)."/../views/v_cit_city.php");
	
	class cit_city extends cit_city_view {
		
		public function __construct()
		{
			parent::__construct();
			
			$this->label 		= gettext("Commune");
			$this->labels 		= gettext("Communes");
			$this->newtext		= gettext("Nouveau Commune");
			$this->picto 		= '<i class="fal fa-city fa-fw"></i>';
			$this->level		= 1;
		...
		public function c_newLine(){
			if($this->checkUserRight()){
				...
				if($this->m_insert($this->values)){
					...
					$this->json['info'] = getText("Commune ajouté");
					// *** view ***
		...
		public function c_search(){
			if($this->checkUserRight()){
				...
				$this->conds = array("LOWER(cit_name) LIKE '$search'");
				$this->dataSent = array();
		...
		public function c_selectSearch(){
			if($this->checkUserRight()){
				...
				$this->fields = $this->key." AS id, CONCAT_WS(' ',cit_pc, cit_name) AS val, '' AS tokens";
				$this->conds = array("LOWER(cit_name) LIKE '$search' OR cit_pc LIKE '$search'");
				$this->dataSent = array();
				if($this->m_getAll(false)){
					// *** view ***
					if($this->count > 0){
			...
			if($this->checkUserRight()){
				...
				if($this->m_update(decrypt($this->idrecord), $data)){
					...
					if($this->m_getById(decrypt($this->idrecord))){
						...
						$this->json['info'] = getText("Commune mis à jour");
						return $this->json;
					}else{
		...
		public function c_delete(){
			if($this->checkUserRight()){
				if($this->m_delete(decrypt($this->idrecord))){
					...
					$this->json['info'] = getText("Commune supprimé");
					return $this->json;
				}else{
