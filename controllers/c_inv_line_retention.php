...
**/
	require_once (dirname(__FILE__)."/../views/v_inv_line_retention.php");
	
	class inv_line_retention extends inv_line_retention_view {
		
		public function __construct()
		{
			parent::__construct();
			
			$this->label 		= gettext("line_retention");
			$this->labels 		= gettext("line_retentions");
			$this->newtext		= gettext("Nouveau line_retention");
		...
		public function c_tableByParent(){
			if($this->checkUserRight()){
				// *** model ***
				$this->conds[] = "inv_line_retention.idinvoice = ".decrypt($this->idrecord);
				if($this->m_getAll()){
					// *** view ***
		...
		public function c_newLine(){
			if($this->checkUserRight()){
				...
				if($this->m_insert($this->values)){
					...
					$this->json['info'] = getText("ligne retenue ajouté");
					// *** view ***
		...
		public function c_insert(){
			if($this->checkUserRight()){
				...
				if($this->m_insert($data)){
					...
					$this->json['idparent'] = encrypt($this->result['idparent']);
					$this->json['info'] = getText("line_retention ajouté");
					return $this->json;
				}else{
			...
			if($this->checkUserRight()){
				...
				if($this->m_update(decrypt($this->idrecord), $data)){
					...
					if($this->m_getById(decrypt($this->idrecord))){
						...
						$this->json['info'] = getText("line_retention mis à jour");
						return $this->json;
					}else{
		...
		public function c_delete(){
			if($this->checkUserRight()){
				if($this->m_delete(decrypt($this->idrecord))){
					...
					$this->json['info'] = getText("line_retention supprimé");
					return $this->json;
				}else{
