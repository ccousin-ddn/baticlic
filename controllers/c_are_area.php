...
**/
	require_once (dirname(__FILE__)."/../views/v_are_area.php");
	
	class are_area extends are_area_view {
		
		public function __construct()
		{
			parent::__construct();
			
			$this->label 		= gettext("Zone");
			$this->labels 		= gettext("Zones");
			$this->newtext		= gettext("Nouvelle Zone");
			$this->picto 		= '<i class="fal fa-dot-circle fa-fw"></i>';
			$this->level		= 1;
		...
		public function c_newLine(){
			if($this->checkUserRight()){
				...
				if($this->m_insert($this->values)){
					...
					$this->json['info'] = getText("Zone ajouté");
					// *** view ***
		...
		public function c_search(){
			if($this->checkUserRight()){
				...
				$this->conds = array("LOWER(are_name) LIKE '$search'");
				$this->dataSent = array();
		...
		public function c_selectSearch(){
			if($this->checkUserRight()){
				...
				$this->fields = $this->key." AS id, are_name AS val, '' AS tokens";
				$this->conds = array("LOWER(are_name) LIKE '$search'");
				$this->dataSent = array();
				if($this->m_getAll(false)){
					// *** view ***
			...
			if($this->checkUserRight()){
				...
				if($this->m_update(decrypt($this->idrecord), $data)){
					...
					if($this->m_getById(decrypt($this->idrecord))){
						...
						$this->json['info'] = getText("Zone mis à jour");
						return $this->json;
					}else{
		...
		public function c_delete(){
			if($this->checkUserRight()){
				if($this->m_delete(decrypt($this->idrecord))){
					...
					$this->json['info'] = getText("Zone supprimé");
					return $this->json;
				}else{
		...
		public function c_getByGeo($lat,$lng){
			$this->m_getByGeo($lat,$lng);
			return $this->values[0]['idarea']??1;
		}
