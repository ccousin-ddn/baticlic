...
**/
	require_once (dirname(__FILE__)."/../views/v_pay_payment.php");
	
	class pay_payment extends pay_payment_view {
		
		public function __construct()
		{
			parent::__construct();
			
			$this->label 		= gettext("Paiement");
			$this->labels 		= gettext("Paiements");
			$this->newtext		= gettext("Nouveau Paiement");
			$this->picto 		= '<i class="fal fa-euro-sign fa-fw"></i>';
			$this->level		= 1;
			...
			$this->pay_type 	= array(1=>'Virement',2=>'Mobile',3=>'Liquide',4=>'Carte',5=>'Chèque');
		}
		...
		public function c_tableByParent(){
			if($this->checkUserRight()){
				// *** model ***
				$this->conds[] = "idinvoice = ".decrypt($this->idrecord);
				if($this->m_getAll()){
					// *** view ***
		...
		public function c_newLine(){
			if($this->checkUserRight()){
				...
				$this->values->pay_amount = $this->dataSent["total"];
				//$this->values = (array) $this->values;
				if($this->m_insert((array) $this->values)){
					$this->json['code'] = "newLineInserted";
					$this->json['idparent'] = encrypt($this->result['idparent']);
					$this->json['info'] = getText("Paiement ajouté");
					// *** view ***
					$this->values->idpayment = $this->result['idparent'];
					$this->v_editLine();
					return $this->json;
				}else{
		...
		public function c_search(){
			if($this->checkUserRight()){
				...
				$this->conds = array("LOWER(pay_name) LIKE '$search'");
				$this->dataSent = array();
		...
		public function c_selectSearch(){
			if($this->checkUserRight()){
				...
				$this->fields = $this->key." AS id, pay_name AS val, '' AS tokens";
				$this->conds = array("LOWER(pay_name) LIKE '$search'");
				$this->dataSent = array();
				if($this->m_getAll(false)){
					// *** view ***
		...
		public function c_insertAndTotal($data){
			if($this->checkUserRight()){
				// *** model ***
				$this->duplicateKey = false;
				if($this->m_insert($data)){
					$this->fields = "SUM(pay_amount) AS tot";
					$this->conds = array("idinvoice = ".$data['idinvoice']);
					$this->select();
					return $this->values[0]['tot'];
				}else{
					throw new Exception('PHP : Error in controller insert', 220);
				}
			}
			...
			if($this->checkUserRight()){
				...
				if($this->m_update(decrypt($this->idrecord), $data)){
					...
					if($this->m_getById(decrypt($this->idrecord))){
						...
						$this->json['info'] = getText("Paiement mis à jour");
						return $this->json;
					}else{
		...
		public function c_delete(){
			if($this->checkUserRight()){
				if($this->m_delete(decrypt($this->idrecord))){
					...
					$this->json['info'] = getText("Paiement supprimé");
					return $this->json;
				}else{
