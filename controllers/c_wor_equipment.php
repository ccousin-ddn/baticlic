...
**/
	require_once (dirname(__FILE__)."/../views/v_wor_equipment.php");
	
	class wor_equipment extends wor_equipment_view {
		
		public function __construct()
		{
			parent::__construct();
			
			$this->label 		= gettext("Equipement");
			$this->labels 		= gettext("Equipements");
			$this->newtext		= gettext("Nouveau Equipement");
			$this->picto 		= '<i class="fal fa-hard-hat fa-fw"></i>';
			$this->level		= 1;
		...
		public function c_tableByParent(){
			if($this->checkUserRight()){
				// *** model ***
				$this->conds[] = "idworker = ".decrypt($this->idrecord);
				if($this->m_getAll()){
					// *** view ***
		...
		public function c_search(){
			if($this->checkUserRight()){
				...
				$this->conds = array("LOWER(equ_name) LIKE '$search'");
				$this->dataSent = array();
		...
		public function c_selectSearch(){
			if($this->checkUserRight()){
				...
				$this->fields = $this->key." AS id, equ_name AS val, '' AS tokens";
				$this->conds = array("LOWER(equ_name) LIKE '$search'");
				$this->dataSent = array();
				if($this->m_getAll(false)){
					// *** view ***
			...
			if($this->checkUserRight()){
				...
				if($this->m_update(decrypt($this->idrecord), $data)){
					...
					if($this->m_getById(decrypt($this->idrecord))){
						...
						$this->json['info'] = getText("Equipement mis à jour");
						return $this->json;
					}else{
		...
		public function c_delete(){
			if($this->checkUserRight()){
				if($this->m_delete(decrypt($this->idrecord))){
					...
					$this->json['info'] = getText("Equipement supprimé");
					return $this->json;
				}else{
