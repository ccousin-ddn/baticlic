...
**/
	require_once (dirname(__FILE__)."/../views/v_quo_comment.php");
	
	class quo_comment extends quo_comment_view {
		
		public function __construct()
		{
			parent::__construct();
			
			$this->label 		= gettext("Commentaire");
			$this->labels 		= gettext("Commentaires");
			$this->newtext		= gettext("Nouveau commentaire");
			$this->picto 		= '<i class="fal fa-comment-alt"></i>';
			$this->level		= 1;
		...
		public function c_newLine(){
			if($this->checkUserRight()){
				...
				if($this->m_insert(http_build_query($this->values))){
					...
					$this->json['info'] = getText("Commentaire ajouté");
					// *** view ***
					$this->values = array($this->values);
					$this->v_createTr();
					return $this->json;
		...
		public function c_search(){
			if($this->checkUserRight()){
				// *** model ***
				$this->conds = array("com_name LIKE '%".$this->dataSent['value']."%'");
				$this->dataSent = array();
			...
			if($this->checkUserRight()){
				...
				if($this->m_update(decrypt($this->idrecord), $this->dataSent)){
					...
					if($this->m_getById(decrypt($this->idrecord))){
						...
						$this->json['info'] = getText("Commentaire mis à jour");
						return $this->json;
					}else{
		...
		public function c_delete(){
			if($this->checkUserRight()){
				if($this->m_delete(decrypt($this->idrecord))){
					...
					$this->json['info'] = getText("Commentaire supprimé");
					return $this->json;
				}else{
