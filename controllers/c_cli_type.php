...
**/
	require_once (dirname(__FILE__)."/../views/v_cli_type.php");
	
	class cli_type extends cli_type_view {
		
		public function __construct()
		{
			parent::__construct();
			
			$this->label 		= gettext("Type de client");
			$this->labels 		= gettext("Type de clients");
		...
		public function c_newLine(){
			if($this->checkUserRight()){
				...
				if($this->m_insert(http_build_query($this->values))){
					...
					$this->json['info'] = getText("Type de client ajouté");
					// *** view ***
					$this->values = array($this->values);
					$this->v_createTr();
					return $this->json;
			...
			if($this->checkUserRight()){
				...
				if($this->m_update(decrypt($this->idrecord), $this->dataSent)){
					...
					if($this->m_getById(decrypt($this->idrecord))){
						...
						$this->json['info'] = getText("Type de client mis à jour");
						return $this->json;
					}else{
		...
		public function c_delete(){
			if($this->checkUserRight()){
				if($this->m_delete(decrypt($this->idrecord))){
					...
					$this->json['info'] = getText("Type de client supprimé");
					return $this->json;
				}else{
