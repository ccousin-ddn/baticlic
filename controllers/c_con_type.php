...
**/
	require_once (dirname(__FILE__)."/../views/v_con_type.php");
	
	class con_type extends con_type_view {
		
		public function __construct()
		{
			parent::__construct();
			
			$this->label 		= gettext("Fonction");
			$this->labels 		= gettext("Fonctions");
			$this->newtext		= gettext("Nouvelle fonction");
			$this->picto 		= '<i class="fal fa-id-badge"></i>';
			$this->level		= 1;
		...
		public function c_newLine(){
			if($this->checkUserRight()){
				...
				if($this->m_insert(http_build_query($this->values))){
					...
					$this->json['info'] = getText("Fonction ajouté");
					// *** view ***
					$this->values = array($this->values);
					$this->v_createTr();
					return $this->json;
			...
			if($this->checkUserRight()){
				...
				if($this->m_update(decrypt($this->idrecord), $this->dataSent)){
					...
					if($this->m_getById(decrypt($this->idrecord))){
						...
						$this->json['info'] = getText("Fonction mis à jour");
						return $this->json;
					}else{
		...
		public function c_delete(){
			if($this->checkUserRight()){
				if($this->m_delete(decrypt($this->idrecord))){
					...
					$this->json['info'] = getText("Fonction supprimé");
					return $this->json;
				}else{
