...
**/
	require_once (dirname(__FILE__)."/../views/v_veh_tire.php");
	
	class veh_tire extends veh_tire_view {
		
		public function __construct()
		{
			parent::__construct();
			
			$this->label 		= gettext("Pneu");
			$this->labels 		= gettext("Pneus");
			$this->newtext		= gettext("Nouveau Pneu");
			$this->picto 		= '<i class="fal fa-tire-flat fa-fw"></i>';
			$this->level		= 1;
			...
			$this->tireAction		= array(0=>"Rien",1=>"Changé",2=>"Permuté");
		}
		...
		public function c_newLine(){
			if($this->checkUserRight()){
				...
				if($this->m_insert($this->values)){
					...
					$this->json['info'] = getText("Pneu ajouté");
					// *** view ***
		...
		public function c_search(){
			if($this->checkUserRight()){
				...
				$this->conds = array("LOWER(tir_name) LIKE '$search'");
				$this->dataSent = array();
		...
		public function c_selectSearch(){
			if($this->checkUserRight()){
				...
				$this->fields = $this->key." AS id, tir_name AS val, '' AS tokens";
				$this->conds = array("LOWER(tir_name) LIKE '$search'");
				$this->dataSent = array();
				if($this->m_getAll(false)){
					// *** view ***
			...
			if($this->checkUserRight()){
				...
				if($this->m_update(decrypt($this->idrecord), $data)){
					...
					if($this->m_getById(decrypt($this->idrecord))){
						...
						$this->json['info'] = getText("Pneu mis à jour");
						return $this->json;
					}else{
		...
		public function c_delete(){
			if($this->checkUserRight()){
				if($this->m_delete(decrypt($this->idrecord))){
					...
					$this->json['info'] = getText("Pneu supprimé");
					return $this->json;
				}else{
