...
**/
	require_once (dirname(__FILE__)."/../views/v_veh_insurance.php");
	
	class veh_insurance extends veh_insurance_view {
		
		public function __construct()
		{
			parent::__construct();
			
			$this->label 		= gettext("Assurance");
			$this->labels 		= gettext("Assurances");
			$this->newtext		= gettext("Nouveau Assurance");
			$this->picto 		= '<i class="fal fa-car-crash fa-fw"></i>';
			$this->level		= 1;
		...
		public function c_newLine(){
			if($this->checkUserRight()){
				...
				if($this->m_insert($this->values)){
					...
					$this->json['info'] = getText("Assurance ajouté");
					// *** view ***
					//$this->v_createTr(array("action"=>true,"edit"=>true,"delete"=>true));
					
					$this->values['idinsurance'] = $this->result['idparent'];
		...
		public function c_search(){
			if($this->checkUserRight()){
				...
				$this->conds = array("LOWER(ins_name) LIKE '$search'");
				$this->dataSent = array();
		...
		public function c_selectSearch(){
			if($this->checkUserRight()){
				...
				$this->fields = $this->key." AS id, ins_name AS val, '' AS tokens";
				$this->conds = array("LOWER(ins_name) LIKE '$search'");
				$this->dataSent = array();
				if($this->m_getAll(false)){
					// *** view ***
			...
			if($this->checkUserRight()){
				...
				if($this->m_update(decrypt($this->idrecord), $data)){
					...
					if($this->m_getById(decrypt($this->idrecord))){
						...
						$this->json['info'] = getText("Assurance mis à jour");
						return $this->json;
					}else{
		...
		public function c_delete(){
			if($this->checkUserRight()){
				if($this->m_delete(decrypt($this->idrecord))){
					...
					$this->json['info'] = getText("Assurance supprimé");
					return $this->json;
				}else{
