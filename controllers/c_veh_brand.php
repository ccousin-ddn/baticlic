...
**/
	require_once (dirname(__FILE__)."/../views/v_veh_brand.php");
	
	class veh_brand extends veh_brand_view {
		
		public function __construct()
		{
			parent::__construct();
			
			$this->label 		= gettext("Marque véhicule");
			$this->labels 		= gettext("Marques véhicule");
			$this->newtext		= gettext("Nouvelle marque véhicule");
			$this->picto 		= '<i class="fal fa-copyright fa-fw"></i>';
			$this->level		= 1;
		...
		public function c_newLine(){
			if($this->checkUserRight()){
				...
				if($this->m_insert($this->values)){
					...
					$this->json['info'] = getText("Marque véhicule ajouté");
					// *** view ***
		...
		public function c_search(){
			if($this->checkUserRight()){
				...
				$this->conds = array("LOWER(bra_name) LIKE '$search'");
				$this->dataSent = array();
		...
		public function c_selectSearch(){
			if($this->checkUserRight()){
				...
				$this->fields = $this->key." AS id, bra_name AS val, '' AS tokens";
				$this->conds = array("LOWER(bra_name) LIKE '$search'");
				$this->dataSent = array();
				if($this->m_getAll(false)){
					// *** view ***
			...
			if($this->checkUserRight()){
				...
				if($this->m_update(decrypt($this->idrecord), $data)){
					...
					if($this->m_getById(decrypt($this->idrecord))){
						...
						$this->json['info'] = getText("Marque véhicule mis à jour");
						return $this->json;
					}else{
		...
		public function c_delete(){
			if($this->checkUserRight()){
				if($this->m_delete(decrypt($this->idrecord))){
					...
					$this->json['info'] = getText("Marque véhicule supprimé");
					return $this->json;
				}else{
