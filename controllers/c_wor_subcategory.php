...
**/
	require_once (dirname(__FILE__)."/../views/v_wor_subcategory.php");
	
	class wor_subcategory extends wor_subcategory_view {
		...
		public function c_insert(){
			if($this->checkUserRight()){
				...
				if($this->m_insert($this->dataSent)){
					...
					$this->json['idparent'] = encrypt($this->result['idparent']);
					$this->json['info'] = getText("Sous-catégorie ajoutée");
					return $this->json;
				}else{
			...
			if($this->checkUserRight()){
				...
				if($this->m_update(decrypt($this->idrecord), $this->dataSent)){
					...
					if($this->m_getById(decrypt($this->idrecord))){
						...
						$this->json['info'] = getText("Sous-catégorie mise à jour");
						return $this->json;
					}else{
		...
		public function c_delete(){
			if($this->checkUserRight()){
				if($this->m_delete(decrypt($this->idrecord))){
					...
					$this->json['info'] = getText("Sous-catégorie supprimée");
					return $this->json;
				}else{
