...
**/
	require_once (dirname(__FILE__)."/../views/v_met_metier.php");
	
	class met_metier extends met_metier_view {
		
		public function __construct()
		{
			parent::__construct();
			
			$this->label 		= gettext("Métier");
			$this->labels 		= gettext("Métiers");
			$this->newtext		= gettext("Nouveau Métier");
			$this->picto 		= '<i class="fal fa-paint-roller fa-fw"></i>';
			$this->level		= 1;
		...
		public function c_newLine(){
			if($this->checkUserRight()){
				...
				if($this->m_insert($this->values)){
					...
					$this->json['info'] = getText("Métier ajouté");
					// *** view ***
		...
		public function c_search(){
			if($this->checkUserRight()){
				...
				$this->conds = array("LOWER(met_name) LIKE '$search'");
				$this->dataSent = array();
		...
		public function c_selectSearch(){
			if($this->checkUserRight()){
				...
				$this->fields = $this->key." AS id, met_name AS val, '' AS tokens";
				$this->conds = array("LOWER(met_name) LIKE '$search'");
				$this->dataSent = array();
				if($this->m_getAll(false)){
					// *** view ***
			...
			if($this->checkUserRight()){
				...
				if($this->m_update(decrypt($this->idrecord), $data)){
					...
					if($this->m_getById(decrypt($this->idrecord))){
						...
						$this->json['info'] = getText("Métier mis à jour");
						return $this->json;
					}else{
		...
		public function c_delete(){
			if($this->checkUserRight()){
				if($this->m_delete(decrypt($this->idrecord))){
					...
					$this->json['info'] = getText("Métier supprimé");
					return $this->json;
				}else{
