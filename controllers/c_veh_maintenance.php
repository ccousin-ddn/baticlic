...
**/
	require_once (dirname(__FILE__)."/../views/v_veh_maintenance.php");
	
	class veh_maintenance extends veh_maintenance_view {
		
		public function __construct()
		{
			parent::__construct();
			
			$this->label 		= gettext("Entretien");
			$this->labels 		= gettext("Entretiens");
			$this->newtext		= gettext("Nouvel Entretien");
			$this->picto 		= '<i class="fal fa-oil-can fa-fw"></i>';
			$this->level		= 1;
			...
			$this->mai_done		= array(0=>"Pas fait",1=>"Fait");
		}
		...
		public function c_tableByParent(){
			if($this->checkUserRight()){
				// *** model ***
				$this->conds[] = "idvehicle = ".decrypt($this->idrecord);
				if($this->m_getAll()){
					// *** view ***
		...
		public function c_search(){
			if($this->checkUserRight()){
				...
				$this->conds = array("LOWER(mai_name) LIKE '$search'");
				$this->dataSent = array();
		...
		public function c_selectSearch(){
			if($this->checkUserRight()){
				...
				$this->fields = $this->key." AS id, mai_name AS val, '' AS tokens";
				$this->conds = array("LOWER(mai_name) LIKE '$search'");
				$this->dataSent = array();
				if($this->m_getAll(false)){
					// *** view ***
		...
		public function c_checkNext(){
			if($this->checkUserRight()){
				// *** model ***
				$this->conds = array("(NOW() >= DATE_SUB(mai_date, INTERVAL 1 MONTH) OR mai_mileage >= veh_mileage)", "mai_done = 0");
				$this->orders = array("mai_date","mai_mileage");
				// *** view ***	
				if($this->select()){
					$this->v_createToast();
					$this->json['code'] = "toastCreated";
				}else{
					throw new Exception('PHP : Error in controller editCardTable', 204);
				}
				return $this->json;
		...
		public function c_createNext(){
			if($this->checkUserRight()){
				// *** model ***
				if($this->m_createNext(decrypt($this->idrecord))){
					$this->json['idparent'] = encrypt($this->result['newId']);
					$this->json['info'] = getText("Entretien créé");
					return $this->json;
				}else{
			...
			if($this->checkUserRight()){
				...
				if($this->m_update(decrypt($this->idrecord), $data)){
					...
					if($this->m_getById(decrypt($this->idrecord))){
						...
						$this->json['info'] = getText("Entretien mis à jour");
						return $this->json;
					}else{
		...
		public function c_delete(){
			if($this->checkUserRight()){
				if($this->m_delete(decrypt($this->idrecord))){
					...
					$this->json['info'] = getText("Entretien supprimé");
					return $this->json;
				}else{
