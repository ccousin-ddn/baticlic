...
**/
	require_once (dirname(__FILE__)."/../views/v_inv_line_ecotax.php");
	
	class inv_line_ecotax extends inv_line_ecotax_view {
		
		public function __construct()
		{
			parent::__construct();
			
			$this->label 		= gettext("Ligne facture ecotaxe");
			$this->labels 		= gettext("Ligne facture ecotaxes");
			$this->newtext		= gettext("Nouveau Ligne facture ecotaxe");
		...
		public function c_newLine(){
			if($this->checkUserRight()){
				...
				$this->values["lin_description"] = "Ecotaxe";
				$this->values["lin_quantity"] = 1;
				$this->values["lin_pu"] = "";
				$this->values["idvat"] = 1;
				if($this->m_insert($this->values)){
					$this->json['code'] = "newLineInserted";
					$this->json['idparent'] = encrypt($this->result['idparent']);
					$this->json['info'] = getText("Ligne facture ecotaxe ajoutée");
					// *** view ***
		...
		public function c_insert(){
			if($this->checkUserRight()){
				...
				if($this->m_insert($data)){
					...
					$this->json['idparent'] = encrypt($this->result['idparent']);
					$this->json['info'] = getText("Ligne facture ecotaxe ajouté");
					return $this->json;
				}else{
		...
		public function c_update(){
			if($this->checkUserRight()){
				// *** model ***
				parse_str($this->dataSent, $data);
				$data['lin_total'] = $data['lin_quantity']*$data['lin_pu'];
				if($this->m_update(decrypt($this->idrecord), $data)){
					// *** new data to update table ***
					if($this->m_getById(decrypt($this->idrecord))){
						// *** view ***
						$this->v_createTr(array("action"=>true,"edit"=>true,"delete"=>true));
						$this->json['info'] = $this->m_getTotalByInvoice(decrypt($data['idparent']));
		...
		public function c_delete(){
			if($this->checkUserRight()){
				if($this->m_delete(decrypt($this->idrecord))){
					...
					$this->json['info'] = $this->m_getTotalByInvoice(decrypt($this->dataSent['idparent']));
					return $this->json;
				}else{
