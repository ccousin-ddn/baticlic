...
**/
	require_once (dirname(__FILE__)."/../views/v_bil_account_lev3.php");
	
	class bil_account_lev3 extends bil_account_lev3_view {
		
		public function __construct()
		{
			parent::__construct();
			
			$this->label 		= gettext("Compte de charge niv. 3");
			$this->labels 		= gettext("Comptes de charge niv. 3");
			$this->newtext		= gettext("Nouveau compte de charge niv. 3");
		...
		public function c_newLine(){
			if($this->checkUserRight()){
				...
				if($this->m_insert(http_build_query($this->values))){
					...
					$this->json['info'] = getText("Comptes de charges ajouté");
					// *** view ***
					$this->values = array($this->values);
					$this->v_createTr();
					return $this->json;
			...
			if($this->checkUserRight()){
				...
				if($this->m_update(decrypt($this->idrecord), $this->dataSent)){
					...
					if($this->m_getById(decrypt($this->idrecord))){
						...
						$this->json['info'] = getText("Comptes de charges mis à jour");
						return $this->json;
					}else{
		...
		public function c_delete(){
			if($this->checkUserRight()){
				if($this->m_delete(decrypt($this->idrecord))){
					...
					$this->json['info'] = getText("Comptes de charges supprimé");
					return $this->json;
				}else{
