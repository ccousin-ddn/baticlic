...
**/
	require_once (dirname(__FILE__)."/../views/v_veh_control.php");
	
	class veh_control extends veh_control_view {
		
		public function __construct()
		{
			parent::__construct();
			
			$this->label 		= gettext("Contrôle");
			$this->labels 		= gettext("Contrôles");
			$this->newtext		= gettext("Nouveau Contrôle");
			$this->picto 		= '<i class="fal fa-ballot-check fa-fw"></i>';
			$this->level		= 1;
			...
			$this->con_type		= array(1=>'Contrôle technique',2=>'Contrôle pollution');
			$this->con_done		= array(0=>"Pas fait",1=>"Fait");
		}
		...
		public function c_newLine(){
			if($this->checkUserRight()){
				...
				if($this->m_insert($this->values)){
					...
					$this->json['info'] = getText("Contrôle ajouté");
					// *** view ***
		...
		public function c_checkNext(){
			if($this->checkUserRight()){
				// *** model ***
				$this->conds = array("NOW() >= DATE_SUB(con_date, INTERVAL 1 MONTH)", "con_done = 0");
				$this->orders = array("con_date");
				//$this->debugging = true;
				// *** view ***	
				if($this->select()){
					$this->v_createToast();
		...
		public function c_createNext(){
			if($this->checkUserRight()){
				...
				if($this->m_createNext(decrypt($this->idrecord))){
					...
					$this->json['info'] = getText("Contrôle créé");
					return $this->json;
				}else{
			...
			if($this->checkUserRight()){
				...
				if($this->m_update(decrypt($this->idrecord), $data)){
					...
					if($this->m_getById(decrypt($this->idrecord))){
						...
						$this->json['info'] = getText("Contrôle mis à jour");
						return $this->json;
					}else{
		...
		public function c_delete(){
			if($this->checkUserRight()){
				if($this->m_delete(decrypt($this->idrecord))){
					...
					$this->json['info'] = getText("Contrôle supprimé");
					return $this->json;
				}else{
