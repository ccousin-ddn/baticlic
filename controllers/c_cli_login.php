...
**/
	require_once (dirname(__FILE__)."/../views/v_cli_login.php");
	
	class cli_login extends cli_login_view {
		
		public function __construct()
		{
			parent::__construct();
			
			$this->label 		= gettext("Login client");
			$this->labels 		= gettext("Login clients");
			...
			$this->log_level	= array(1=>"Utilisateur",2=>"Super utilisateur",3=>"Administrateur");
		}
		...
		public function c_tableByParent(){
			if($this->checkUserRight()){
				// *** model ***
				$this->conds = array("cli_login.idclient = ".decrypt($this->idrecord));
		...
		public function c_newLine(){
			if($this->checkUserRight()){
				...
				if($this->m_insert(http_build_query($this->values))){
					...
					$this->json['info'] = getText("Login client ajouté");
					// *** view ***
					$this->values = array($this->values);
					$this->v_createTr();
					return $this->json;
		...
		public function c_search(){
			if($this->checkUserRight()){
				// *** model ***
				$this->conds = array("log_name LIKE '%".$this->dataSent['value']."%'");
				$this->dataSent = array();
				if($this->m_getAll(false)){
					// *** view ***
					$this->v_createFullTable();
			...
			if($this->checkUserRight()){
				...
				if($this->m_update(decrypt($this->idrecord), $this->dataSent)){
					...
					if($this->m_getById(decrypt($this->idrecord))){
						...
						$this->json['info'] = getText("Login client mis à jour");
						return $this->json;
					}else{
		...
		public function c_delete(){
			if($this->checkUserRight()){
				if($this->m_delete(decrypt($this->idrecord))){
					...
					$this->json['info'] = getText("Login client supprimé");
					return $this->json;
				}else{
