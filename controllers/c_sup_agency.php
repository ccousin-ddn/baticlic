...
**/
	require_once (dirname(__FILE__)."/../views/v_sup_agency.php");
	
	class sup_agency extends sup_agency_view {
		
		public function __construct()
		{
			parent::__construct();
			
			$this->label 		= gettext("Agence");
			$this->labels 		= gettext("Agences");
			$this->newtext		= gettext("Nouvelle Agence");
			$this->picto 		= '<i class="fal fa-store-alt fa-fw"></i>';
			$this->level		= 1;
		...
		public function c_tableByParent(){
			if($this->checkUserRight()){
				// *** model ***
				$this->conds[] = "idsupplier = ".decrypt($this->idrecord);
				if($this->m_getAll()){
					// *** view ***
		...
		public function c_newLine(){
			if($this->checkUserRight()){
				...
				if($this->m_insert($this->values)){
					...
					$this->json['info'] = getText("Agence ajouté");
					// *** view ***
		...
		public function c_insert(){
			if($this->checkUserRight()){
				...
				if($this->m_insert($data)){
					...
					$this->json['idparent'] = encrypt($this->result['idparent']);
					$this->json['info'] = getText("Agence ajoutée");
					return $this->json;
				}else{
			...
			if($this->checkUserRight()){
				...
				if($this->m_update(decrypt($this->idrecord), $data)){
					...
					if($this->m_getById(decrypt($this->idrecord))){
						...
						$this->json['info'] = getText("Agence mise à jour");
						return $this->json;
					}else{
		...
		public function c_delete(){
			if($this->checkUserRight()){
				if($this->m_delete(decrypt($this->idrecord))){
					...
					$this->json['info'] = getText("Agence supprimée");
					return $this->json;
				}else{
