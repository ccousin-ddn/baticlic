...
**/
	require_once (dirname(__FILE__)."/../views/v_ret_retention.php");
	
	class ret_retention extends ret_retention_view {
		
		public function __construct()
		{
			parent::__construct();
			
			$this->label 		= gettext("Retenue");
			$this->labels 		= gettext("Retenues");
			$this->newtext		= gettext("Nouvelle Retenue");
			$this->picto 		= '<i class="fal fa-envelope-open-dollar fa-fw"></i>';
			$this->level		= 1;
		...
		public function c_newLine(){
			if($this->checkUserRight()){
				...
				if($this->m_insert($this->values)){
					...
					$this->json['info'] = getText("Retenue ajouté");
					// *** view ***
		...
		public function c_search(){
			if($this->checkUserRight()){
				...
				$this->conds = array("LOWER(ret_name) LIKE '$search'");
				$this->dataSent = array();
		...
		public function c_selectSearch(){
			if($this->checkUserRight()){
				...
				$this->fields = $this->key." AS id, ret_name AS val, '' AS tokens";
				$this->conds = array("LOWER(ret_name) LIKE '$search'");
				$this->dataSent = array();
				if($this->m_getAll(false)){
					// *** view ***
		...
		public function c_insert(){
			if($this->checkUserRight()){
				...
				if($this->m_insert($data)){
					...
					$this->json['idparent'] = encrypt($this->result['idparent']);
					$this->json['info'] = getText("Retenue ajoutée");
					return $this->json;
				}else{
			...
			if($this->checkUserRight()){
				...
				if($this->m_update(decrypt($this->idrecord), $data)){
					...
					if($this->m_getById(decrypt($this->idrecord))){
						...
						$this->json['info'] = getText("Retenue mise à jour");
						return $this->json;
					}else{
		...
		public function c_delete(){
			if($this->checkUserRight()){
				if($this->m_delete(decrypt($this->idrecord))){
					...
					$this->json['info'] = getText("Retenue supprimée");
					return $this->json;
				}else{
