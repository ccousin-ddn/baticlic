...
**/
	require_once (dirname(__FILE__)."/../views/v_veh_full.php");
	
	class veh_full extends veh_full_view {
		
		public function __construct()
		{
			parent::__construct();
			
			$this->label 		= gettext("Carburant");
			$this->labels 		= gettext("Carburants");
			$this->newtext		= gettext("Nouveau Carburant");
			$this->picto 		= '<i class="fal fa-gas-pump fa-fw"></i>';
			$this->level		= 1;
		...
		public function c_newLine(){
			if($this->checkUserRight()){
				...
				if($this->m_insert($this->values)){
					...
					$this->json['info'] = getText("Carburant ajouté");
					// *** view ***
		...
		public function c_search(){
			if($this->checkUserRight()){
				...
				$this->conds = array("LOWER(ful_name) LIKE '$search'");
				$this->dataSent = array();
		...
		public function c_selectSearch(){
			if($this->checkUserRight()){
				...
				$this->fields = $this->key." AS id, ful_name AS val, '' AS tokens";
				$this->conds = array("LOWER(ful_name) LIKE '$search'");
				$this->dataSent = array();
				if($this->m_getAll(false)){
					// *** view ***
		...
		public function c_insertFromWarehouse(){
			if($this->checkUserRight()){
				...
				$this->duplicateKey = false;
				$data = $this->dataSent;
				$data['ful_date'] = date("d-m-Y");
				$data['iduser'] = $_SESSION['iduser'];
				
				if($this->m_insert($data)){
					// update stock
					$stock = new sto_stock();
					$stock->m_updateQty($data["idShop"], $data["idArticle"], $data["ful_liter"]);
					
					// insert movement
					$mvt = new mov_movement();
					$data_mvt = array("idshop"=>$data["idShop"], "idarticle"=>$data["idArticle"], "mov_quantity"=>$data["ful_liter"], "mov_date"=>date("d-m-Y"), "iduser"=>$_SESSION['iduser'], "mov_type"=>2, "mov_place"=>5);
					$mvt->insert($data_mvt);
					
					$this->json['code'] = "fullInserted";
					$this->json['idparent'] = encrypt($this->result['idparent']);
					$this->json['info'] = getText("Carburant ajouté");
					return $this->json;
				}else{
			...
			if($this->checkUserRight()){
				...
				if($this->m_update(decrypt($this->idrecord), $data)){
					...
					if($this->m_getById(decrypt($this->idrecord))){
						...
						$this->json['info'] = getText("Carburant mis à jour");
						return $this->json;
					}else{
		...
		public function c_delete(){
			if($this->checkUserRight()){
				if($this->m_delete(decrypt($this->idrecord))){
					...
					$this->json['info'] = getText("Carburant supprimé");
					return $this->json;
				}else{
