...
**/
	require_once (dirname(__FILE__)."/../views/v_wor_team.php");
	
	class wor_team extends wor_team_view {
		
		public function __construct()
		{
			parent::__construct();
			
			$this->label 		= gettext("Equipe");
			$this->labels 		= gettext("Equipes");
			$this->newtext		= gettext("Nouvelle équipe");
			$this->picto 		= '<i class="fal fa-people-carry"></i>';
			$this->level		= 1;
			
			$this->cryptfields	= array("");
			
			$this->lnk			= array("lnk_team_worker"); // GROUP_CONCAT(idlnk) idlnk
		...
		public function c_newLine(){
			if($this->checkUserRight()){
				...
				if($this->m_insert(http_build_query($this->values))){
					...
					$this->json['info'] = getText("Equipe ajouté");
					// *** view ***
					$this->values = array($this->values);
					$this->v_createTr();
					return $this->json;
		...
		public function c_search(){
			if($this->checkUserRight()){
				// *** model ***
				$this->conds = array("tea_name LIKE '%".$this->dataSent['value']."%'");
				$this->dataSent = array();
			...
			if($this->checkUserRight()){
				...
				if($this->m_update(decrypt($this->idrecord), $this->dataSent)){
					...
					if($this->m_getById(decrypt($this->idrecord))){
						...
						$this->json['info'] = getText("Equipe mis à jour");
						return $this->json;
					}else{
		...
		public function c_delete(){
			if($this->checkUserRight()){
				if($this->m_delete(decrypt($this->idrecord))){
					...
					$this->json['info'] = getText("Equipe supprimé");
					return $this->json;
				}else{
