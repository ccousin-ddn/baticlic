...
**/
	require_once (dirname(__FILE__)."/../views/v_vat_vat.php");
	
	class vat_vat extends vat_vat_view {
		
		public function __construct()
		{
			parent::__construct();
			
			$this->label 		= gettext("Taux TVA");
			$this->labels 		= gettext("Taux TVA");
			$this->newtext		= gettext("Nouveau Taux");
			$this->picto 		= '<i class="fal fa-percent fa-fw"></i>';
			$this->level		= 1;
		...
		public function c_newLine(){
			if($this->checkUserRight()){
				...
				if($this->m_insert($this->values)){
					...
					$this->json['info'] = getText("TVA ajouté");
					// *** view ***
		...
		public function c_search(){
			if($this->checkUserRight()){
				...
				$this->conds = array("LOWER(vat_name) LIKE '$search'");
				$this->dataSent = array();
		...
		public function c_selectSearch(){
			if($this->checkUserRight()){
				...
				$this->fields = $this->key." AS id, vat_name AS val, '' AS tokens";
				$this->conds = array("LOWER(vat_name) LIKE '$search'");
				$this->dataSent = array();
				if($this->m_getAll(false)){
					// *** view ***
			...
			if($this->checkUserRight()){
				...
				if($this->m_update(decrypt($this->idrecord), $data)){
					...
					if($this->m_getById(decrypt($this->idrecord))){
						...
						$this->json['info'] = getText("TVA mis à jour");
						return $this->json;
					}else{
		...
		public function c_delete(){
			if($this->checkUserRight()){
				if($this->m_delete(decrypt($this->idrecord))){
					...
					$this->json['info'] = getText("TVA supprimé");
					return $this->json;
				}else{
