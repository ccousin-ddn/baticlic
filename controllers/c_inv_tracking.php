...
**/
	require_once (dirname(__FILE__)."/../views/v_inv_tracking.php");
	
	class inv_tracking extends inv_tracking_view {
		
		public function __construct()
		{
			parent::__construct();
			
			$this->label 		= gettext("tracking");
			$this->labels 		= gettext("trackings");
			$this->newtext		= gettext("Nouveau tracking");
		...
		public function c_newLine(){
			if($this->checkUserRight()){
				...
				if($this->m_insert((array) $this->values)){
					...
					$this->json['info'] = getText("tracking ajouté");
					// *** view ***
					$this->values->idtracking = $this->result['idparent'];
					$this->v_editLine();
					return $this->json;
				}else{
		...
		public function c_search(){
			if($this->checkUserRight()){
				...
				$this->conds = array("LOWER(tra_name) LIKE '$search'");
				$this->dataSent = array();
		...
		public function c_selectSearch(){
			if($this->checkUserRight()){
				...
				$this->fields = $this->key." AS id, tra_name AS val, '' AS tokens";
				$this->conds = array("LOWER(tra_name) LIKE '$search'");
				$this->dataSent = array();
				if($this->m_getAll(false)){
					// *** view ***
			...
			if($this->checkUserRight()){
				...
				if($this->m_update(decrypt($this->idrecord), $data)){
					...
					if($this->m_getById(decrypt($this->idrecord))){
						...
						$this->json['info'] = getText("tracking mis à jour");
						return $this->json;
					}else{
		...
		public function c_delete(){
			if($this->checkUserRight()){
				if($this->m_delete(decrypt($this->idrecord))){
					...
					$this->json['info'] = getText("tracking supprimé");
					return $this->json;
				}else{
