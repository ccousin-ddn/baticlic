...
**/
	require_once (dirname(__FILE__)."/../views/v_wor_medical.php");
	
	class wor_medical extends wor_medical_view {
		
		public function __construct()
		{
			parent::__construct();
			
			$this->label 		= gettext("Visite médicale");
			$this->labels 		= gettext("Visites médicales");
			$this->newtext		= gettext("Nouvelle visite médicale");
			$this->picto 		= '<i class="fal fa-user-md fa-fw"></i>';
			$this->level		= 0;
		...
		public function c_newLine(){
			if($this->checkUserRight()){
				...
				if($this->m_insert($this->values)){
					...
					$this->json['info'] = getText("Visite médicale ajouté");
					// *** view ***
		...
		public function c_search(){
			if($this->checkUserRight()){
				...
				$this->conds = array("LOWER(med_name) LIKE '$search'");
				$this->dataSent = array();
		...
		public function c_selectSearch(){
			if($this->checkUserRight()){
				...
				$this->fields = $this->key." AS id, med_name AS val, '' AS tokens";
				$this->conds = array("LOWER(med_name) LIKE '$search'");
				$this->dataSent = array();
				if($this->m_getAll(false)){
					// *** view ***
		...
		public function c_checkNext(){
			if($this->checkUserRight()){
				// *** model ***
				$this->conds = array("NOW() >= DATE_SUB(med_next_date, INTERVAL 1 MONTH)");
				$this->orders = array("med_next_date");
				// *** view ***	
				if($this->select()){
					$this->v_createToast();
			...
			if($this->checkUserRight()){
				...
				if($this->m_update(decrypt($this->idrecord), $data)){
					...
					if($this->m_getById(decrypt($this->idrecord))){
						...
						$this->json['info'] = getText("Visite médicale mis à jour");
						return $this->json;
					}else{
		...
		public function c_delete(){
			if($this->checkUserRight()){
				if($this->m_delete(decrypt($this->idrecord))){
					...
					$this->json['info'] = getText("Visite médicale supprimé");
					return $this->json;
				}else{
