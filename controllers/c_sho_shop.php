...
**/
	require_once (dirname(__FILE__)."/../views/v_sho_shop.php");
	
	class sho_shop extends sho_shop_view {
		
		public function __construct()
		{
			parent::__construct();
			
			$this->label 		= gettext("Magasin");
			$this->labels 		= gettext("Magasins");
			$this->newtext		= gettext("Nouveau magasin");
			...
			$this->sho_type		= array(1=>"Matériaux(10)",2=>"Matériel(20)",3=>"Outillage(30)",4=>"Equipement(40)",5=>"Carburant(50)");
		}
		...
		public function c_newLine(){
			if($this->checkUserRight()){
				...
				if($this->m_insert(http_build_query($this->values))){
					...
					$this->json['info'] = getText("Magasin ajouté");
					// *** view ***
					$this->values = array($this->values);
					$this->v_createTr();
					return $this->json;
		...
		public function c_search(){
			if($this->checkUserRight()){
				// *** model ***
				$this->conds = array("sho_name LIKE '%".$this->dataSent['value']."%'");
				$this->dataSent = array();
		...
		public function c_showCard(){
			if($this->checkUserRight()){
				...
				$sto_stock->idrecord = $this->idrecord;
				$sto_stock->c_tableByParent();
				
				// *** view ***	
				if($res){
					$this->v_createCard($sto_stock);
					$this->json['code'] = 1;
					return $this->json;
				}else{
		...
		public function c_getType(){
			// *** model ***
			//$this->fields = "";
			//$this->joins = "";
			$res = $this->m_getById($this->idrecord);
			$this->values = (object) $this->values[0];
			
			// *** view ***	
			if($res){
				return $this->values->sho_type;
			}else{
				throw new Exception('PHP : Error in controller pdf', 212);
			}
		}
		
		public function c_stockGetByType(){
			if($this->checkUserRight()){
				// *** model ***
				$this->conds = array("sho_type IN (".$this->dataSent['sho_type'].")");
				$this->dataSent = array();
				if($this->m_getAll(false)){
					// *** view ***
			...
			if($this->checkUserRight()){
				...
				if($this->m_update(decrypt($this->idrecord), $this->dataSent)){
					...
					if($this->m_getById(decrypt($this->idrecord))){
						...
						$this->json['info'] = getText("Magasin mis à jour");
						return $this->json;
					}else{
		...
		public function c_delete(){
			if($this->checkUserRight()){
				if($this->m_delete(decrypt($this->idrecord))){
					...
					$this->json['info'] = getText("Magasin supprimé");
					return $this->json;
				}else{
