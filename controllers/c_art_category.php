...
**/
	require_once (dirname(__FILE__)."/../views/v_art_category.php");
	
	class art_category extends art_category_view {
		
		public function __construct()
		{
			parent::__construct();
			
			$this->label 		= gettext("Catégorie");
			$this->labels 		= gettext("Catégories");
			$this->newtext		= gettext("Nouvelle catégorie");
		...
		public function c_newLine(){
			if($this->checkUserRight()){
				...
				if($this->m_insert(http_build_query($this->values))){
					...
					$this->json['info'] = getText("Catégorie ajouté");
					// *** view ***
					$this->values = array($this->values);
					$this->v_createTr();
					return $this->json;
		...
		public function c_search(){
			if($this->checkUserRight()){
				// *** model ***
				$this->conds = array("cat_name LIKE '%".$this->dataSent['value']."%'");
				$this->dataSent = array();
			...
			if($this->checkUserRight()){
				...
				if($this->m_update(decrypt($this->idrecord), $this->dataSent)){
					...
					if($this->m_getById(decrypt($this->idrecord))){
						...
						$this->json['info'] = getText("Catégorie mis à jour");
						return $this->json;
					}else{
		...
		public function c_delete(){
			if($this->checkUserRight()){
				if($this->m_delete(decrypt($this->idrecord))){
					...
					$this->json['info'] = getText("Catégorie supprimé");
					return $this->json;
				}else{
