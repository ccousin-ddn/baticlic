...
**/
	require_once (dirname(__FILE__)."/../views/v_tas_type.php");
	
	class tas_type extends tas_type_view {
		
		public function __construct()
		{
			parent::__construct();
			
			$this->label 		= gettext("Type de poste");
			$this->labels 		= gettext("Types de poste");
		...
		public function c_newLine(){
			if($this->checkUserRight()){
				...
				if($this->m_insert(http_build_query($this->values))){
					...
					$this->json['info'] = getText("Type de tâcher ajouté");
					// *** view ***
					$this->values = array($this->values);
					$this->v_createTr();
					return $this->json;
			...
			if($this->checkUserRight()){
				...
				if($this->m_update(decrypt($this->idrecord), $this->dataSent)){
					...
					if($this->m_getById(decrypt($this->idrecord))){
						...
						$this->json['info'] = getText("Type de tâcher mis à jour");
						return $this->json;
					}else{
		...
		public function c_delete(){
			if($this->checkUserRight()){
				if($this->m_delete(decrypt($this->idrecord))){
					...
					$this->json['info'] = getText("Type de tâcher supprimé");
					return $this->json;
				}else{
